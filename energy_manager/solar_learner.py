...
MIN_PHASE_DETECTIONS      = 5      # Aantal consistente waarnemingen voor "zeker"
...
class PhaseVotes:
    ...
    def best_guess(self) -> tuple[str | None, float]:
        """Return (best_phase, confidence_pct) even with few votes.

        confidence_pct = fraction of votes for the leading phase (0–100).
        Returns (None, 0.0) if no votes at all.
        """
        total = self.total()
        if total == 0:
            return None, 0.0
        best = max(("L1", "L2", "L3"), key=lambda p: getattr(self, p))
        conf = round(getattr(self, best) / total * 100, 1)
        return best, conf
...
class SolarPowerLearner:
    ...
    async def _detect_phase(
        ...
        _LOGGER.info(
            "SolarLearner '%s': fase-stem %d/%d [%s] — %s wint met ΔPV=+%.0fW Δ%s=+%.2fA | stemmen: %s",
            profile.label, votes.total(), MIN_PHASE_DETECTIONS, bar,
            winner, power_delta, winner, phase_deltas[winner], votes.to_dict(),
        )
    ...
    def _phase_display_fields(p: "InverterProfile") -> dict:
        """Return phase_display + phase_confidence for any confidence level."""
        votes: PhaseVotes = p._votes_obj or PhaseVotes()
        if p.phase_certain and p.detected_phase:
            return {
                "phase_display":     p.detected_phase,
                "phase_confidence":  100.0,
