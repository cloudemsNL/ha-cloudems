...
class DayTypeClassifier:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._dirty      = False
        self._last_save  = 0.0
        # Weekdag-specifieke piekuren: weekday (0=Ma..6=Zo) → {uur: gewogen_teller}
        self._weekday_peaks: dict[int, dict[int, float]] = {d: {} for d in range(7)}
    ...
    async def async_setup(self) -> None:
        ...
        # Weekdag-peaks herladen (keys zijn strings in JSON)
        wp_raw = saved.get("weekday_peaks", {})
        for d in range(7):
            self._weekday_peaks[d] = {int(h): float(v) for h, v in wp_raw.get(str(d), {}).items()}
    ...
    def observe_power(self, power_w: float) -> None:
        ...
        # Weekdag-specifieke piekuur tracking (EMA per weekdag × uur)
        dow = now.weekday()   # 0=Ma..6=Zo
        peaks = self._weekday_peaks[dow]
        current = peaks.get(hour, 0.0)
        peaks[hour] = 0.90 * current + 0.10 * power_w
        self._dirty = True
    ...
    def get_peak_hours(self, weekday: int, top_n: int = 3) -> list[int]:
        """Geef de top-N piekuren voor een specifieke weekdag (0=Ma..6=Zo).

        Geeft een lege lijst als er nog te weinig data is.
        Gebruikt door LoadPlanner voor dag-type correctie.
        """
        peaks = self._weekday_peaks.get(weekday % 7, {})
        if not peaks or self._total_days < 7:
            return []
        sorted_hours = sorted(peaks.items(), key=lambda x: x[1], reverse=True)
    ...
    async def async_maybe_save(self) -> None:
        if self._dirty and (time.time() - self._last_save) >= SAVE_INTERVAL_S:
            await self._store.async_save({
                ...
                "weekday_peaks": {str(d): v for d, v in self._weekday_peaks.items()},
            })
            self._dirty     = False
