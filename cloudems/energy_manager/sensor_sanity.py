...
class SensorSanityGuard:
    ...
    _STORE_KEY     = "cloudems_sensor_sanity_v1"
    _STORE_VERSION = 1
    ...
    def __init__(self, config: dict, hass=None):
        ...
        # Persistence: saved means so spike-detection works immediately after restart
    ...
    async def async_setup(self) -> None:
        """Laad geleerde sensor-gemiddelden vanuit opslag."""
        if not self._hass:
            return
        ...
        try:
            ...
            self._learned_means = {k: float(v) for k, v in data.get("means", {}).items()}
            import logging as _l
            _l.getLogger(__name__).debug(
                "SensorSanityGuard: %d geleerde gemiddelden geladen", len(self._learned_means)
            )
        except Exception as exc:
            import logging as _l
            _l.getLogger(__name__).warning("SensorSanityGuard: laden mislukt: %s", exc)

    async def _async_save(self) -> None:
        import time as _t
        if not self._store or not self._dirty or _t.time() - self._last_save < 300:
            return
        try:
            await self._store.async_save({"means": self._learned_means})
            self._dirty = False
            self._last_save = _t.time()
        except Exception as exc:
            import logging as _l
            _l.getLogger(__name__).warning("SensorSanityGuard: opslaan mislukt: %s", exc)
    ...
    def update(
        ...
        for name, eid, val in [("grid", grid_eid, grid_w), ("solar", solar_eid, solar_w)]:
            if val is None:
                continue
            hist = self._get_history(eid)
            # v1.32: seed history met geleerd gemiddelde bij eerste gebruik na herstart
            if eid not in self._history and eid in self._learned_means:
                seed = self._learned_means[eid]
                for _ in range(min(30, HISTORY_LEN)):
                    hist.append(seed)
            self._push_history(eid, val)
            if len(hist) >= 30:
                mean = sum(abs(h) for h in hist) / len(hist)
                # Update persistent mean (EMA)
                prev = self._learned_means.get(eid)
