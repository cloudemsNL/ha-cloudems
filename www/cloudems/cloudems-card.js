...
class CloudEMSCard extends LitElement {
  ...
  _closeNilmPopup() {
    ...
    const fmtPw = pw != null ? (pw >= 1000 ? (pw/1000).toFixed(1)+"kW" : Math.round(pw)+"W") : null;

    // Bouw popup via DOM API zodat event listeners werken naast Shadow DOM
    const el = document.createElement("div");
    el.style.cssText = "position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.65);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;";

    const card = document.createElement("div");
    card.style.cssText = "background:#16161e;border:1px solid rgba(255,255,255,.1);border-radius:16px;padding:18px;width:calc(100% - 48px);max-width:320px;box-shadow:0 20px 60px rgba(0,0,0,.7);font-family:ui-sans-serif,sans-serif;font-size:14px;color:#e2e8f0;";

    const hdrDiv = document.createElement("div");
    hdrDiv.style.cssText = "display:flex;align-items:center;gap:12px;margin-bottom:12px;";
    hdrDiv.innerHTML = `<div style="width:40px;height:40px;border-radius:10px;background:${icon.color}1a;border:1px solid ${icon.color}33;display:flex;align-items:center;justify-content:center;font-size:1.4rem;flex-shrink:0;">${icon.emoji}</div><div style="flex:1;min-width:0;"><div style="font-size:.9rem;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</div><div style="font-size:.68rem;color:${icon.color};">${icon.label}${fmtPw ? " � "+fmtPw : ""}</div></div>`;
    const btnClose = document.createElement("button");
    btnClose.textContent = "";
    btnClose.style.cssText = "background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#94a3b8;cursor:pointer;font-size:.9rem;padding:5px 9px;flex-shrink:0;";
    btnClose.addEventListener("click", () => this._closeNilmPopup());
    hdrDiv.appendChild(btnClose);
    card.appendChild(hdrDiv);

    const badgesDiv = document.createElement("div");
    badgesDiv.style.cssText = "display:flex;flex-wrap:wrap;gap:5px;margin-bottom:10px;";
    badgesDiv.innerHTML = `<span style="background:${phStyle.bg};border:1px solid ${phStyle.border};color:${phStyle.text};border-radius:5px;padding:2px 7px;font-size:.68rem;">� ${phStyle.label}</span><span style="background:${srcStyle.bg}22;border:1px solid ${srcStyle.bg}88;color:${srcStyle.bg};border-radius:5px;padding:2px 7px;font-size:.68rem;">${srcStyle.ico} ${srcStyle.label}</span>${isConfirmed ? '<span style="background:#22c55e20;border:1px solid #22c55e60;color:#4ade80;border-radius:5px;padding:2px 7px;font-size:.68rem;"> Goedgekeurd</span>' : ''}${isSuppressed ? '<span style="background:#ef444420;border:1px solid #ef444460;color:#f87171;border-radius:5px;padding:2px 7px;font-size:.68rem;">=� Onderdrukt</span>' : ''}`;
    card.appendChild(badgesDiv);

    if (!isPlug && conf !== null) {
      const confDiv = document.createElement("div");
      confDiv.style.cssText = "margin-bottom:10px;";
      confDiv.innerHTML = `<div style="display:flex;justify-content:space-between;font-size:.65rem;color:#64748b;margin-bottom:3px;"><span>Betrouwbaarheid</span><span style="color:${confColor};">${conf}%</span></div><div style="background:rgba(255,255,255,.08);border-radius:3px;height:3px;"><div style="width:${conf}%;height:100%;background:${confColor};border-radius:3px;"></div></div>`;
      card.appendChild(confDiv);
    }

    const renameDiv = document.createElement("div");
    renameDiv.style.cssText = "display:flex;gap:6px;align-items:center;margin-bottom:10px;";
    const renameInput = document.createElement("input");
    renameInput.type = "text";
    renameInput.value = name;
    renameInput.placeholder = "Naam apparaat&";
    renameInput.style.cssText = "flex:1;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);border-radius:8px;color:#e2e8f0;padding:7px 10px;font-size:.8rem;outline:none;min-width:0;";
    const btnRename = document.createElement("button");
    btnRename.textContent = " Opslaan";
    btnRename.style.cssText = "background:#3b82f614;border:1px solid #3b82f640;border-radius:8px;color:#93c5fd;cursor:pointer;font-size:.78rem;font-weight:600;padding:7px 11px;white-space:nowrap;flex-shrink:0;";
    btnRename.addEventListener("click", () => {
      const newName = renameInput.value.trim();
      if (newName && newName !== name) this._nilmAction("rename_device", { ...d, new_name: newName });
      else this._closeNilmPopup();
    });
    renameDiv.appendChild(renameInput);
    renameDiv.appendChild(btnRename);
    card.appendChild(renameDiv);

    const divider = document.createElement("div");
    divider.style.cssText = "border-top:1px solid rgba(255,255,255,.08);margin:4px 0 12px;";
    card.appendChild(divider);

    const actionsDiv = document.createElement("div");
    actionsDiv.style.cssText = "display:flex;flex-direction:column;gap:8px;";

    const mkBtn = (emoji, label, css) => {
      const b = document.createElement("button");
      b.innerHTML = emoji + " " + label;
      b.style.cssText = `display:flex;align-items:center;gap:8px;width:100%;padding:10px 14px;border-radius:10px;cursor:pointer;font-size:.82rem;font-weight:600;${css}`;
      return b;
    };

    if (!isConfirmed) {
      const bA = mkBtn("", "Goedkeuren", "border:1px solid #22c55e40;background:#22c55e14;color:#4ade80;");
      bA.addEventListener("click", () => this._nilmAction("confirm_device", d));
      actionsDiv.appendChild(bA);
    }
    if (!isSuppressed) {
      const bS = mkBtn("=�", "Onderdrukken", "border:1px solid #ef444440;background:#ef444414;color:#f87171;");
      bS.addEventListener("click", () => this._nilmAction("suppress_nilm_device", d));
      actionsDiv.appendChild(bS);
    } else {
      const bU = mkBtn("�", "Onderdrukking opheffen", "border:1px solid #3b82f640;background:#3b82f614;color:#93c5fd;");
      bU.addEventListener("click", () => this._nilmAction("confirm_device", d));
      actionsDiv.appendChild(bU);
    }
    const bD = mkBtn("=�", "Verwijderen", "border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);color:#64748b;");
    bD.addEventListener("click", () => this._nilmAction("dismiss_device", d));
    actionsDiv.appendChild(bD);
    card.appendChild(actionsDiv);

    const hint = document.createElement("div");
    hint.style.cssText = "font-size:.64rem;color:#64748b;margin-top:10px;text-align:center;";
    hint.textContent = "Klik buiten dit venster om te sluiten";
    card.appendChild(hint);

    el.appendChild(card);
    el.addEventListener("click", (e) => { if (e.target === el) this._closeNilmPopup(); });
  ...
  async _nilmAction(action, device) {
    ...
    // confirm_device vereist ook device_type en name
    const payload = { device_id: deviceId };
    if (action === "confirm_device") {
      payload.device_type = device.device_type || device.type || "unknown";
      payload.name = device.name || payload.device_type;
    }
    if (action === "rename_device") {
      payload.name = device.new_name;
      payload.device_type = device.device_type || device.type || "unknown";
    }
    try {
  ...
  _renderDevices() {
    ...
    // v1.20.2: sensor gebruikt korte sleutelnames (on/w/ph/src/c/ok/sup/id) om onder 16KB te blijven
    const statsSensor = nilmEid.replace("_running_devices", "_devices");
    const allDevicesRaw = this._attr(statsSensor, "devices", null);
    let detected = [];
    if (allDevicesRaw) {
      detected = allDevicesRaw.filter(d =>
        !d.on && !d.dismissed &&
        (d.src === "smart_plug" || (d.c || 0) > 0)
      ).map(d => ({
        name:        d.name || d.type,
        type:        d.type,
        device_type: d.type,
        device_id:   d.id || d.name,
        state:       "off",
        running:     false,
        power_w:     d.w || 0,
        confidence:  d.c || 0,
        ...
        phase_label: d.ph || "L1",
        source:      d.src || "nilm",
        source_type: d.src || "nilm",
        confirmed:   (d.ok || 0) === 1,
        user_suppressed: (d.sup || 0) === 1,
      }));
    }
    ...
    return html`
      ...
      ${detected.length > 0 ? html`
        ...
        ${detected.length > 2 ? (() => {
          const dp={L1:0,L2:0,L3:0};
          detected.forEach(d=>{const ph=d.phase||"L1";if(ph in dp)dp[ph]++;});
          if(!Object.values(dp).some(v=>v>0)) return "";
          return html`<div class="phase-summary" style="margin-bottom:6px;opacity:.75;">
            ${["L1","L2","L3"].map(ph=>{
              const ps=this._phaseStyle(ph);
              return dp[ph]>0?html`<span class="ph-pill" style="background:${ps.bg};border-color:${ps.border};color:${ps.text}">${ps.label} � ${dp[ph]}</span>`:``;
            })}
          </div>`;
        })() : ""}
  ...
  _renderInvRow(inv) {
    ...
    // 3-fasen PV fase detectie (v1.20.2)
    const invPhase     = s?.attributes?.phase ?? null;
    const phaseCertain = s?.attributes?.phase_certain ?? false;
    const phaseConf    = s?.attributes?.phase_confidence ?? 0;
    const isThreePhase = invPhase === "ALL";
    const phStyle3     = invPhase && !isThreePhase ? this._phaseStyle(invPhase) : null;
    const phaseBadge   = invPhase && invPhase !== "unknown"
      ? (isThreePhase
          ? html`<span style="display:inline-flex;align-items:center;gap:4px;background:#a855f720;border:1px solid #a855f750;border-radius:99px;padding:2px 8px;margin-left:4px" title="3-fasen omvormer"><span style="font-size:0.65rem;color:#d8b4fe;font-weight:600">3 ${phaseCertain ? "" : "~"}</span></span>`
          : html`<span style="display:inline-flex;align-items:center;gap:4px;background:${phStyle3?.bg};border:1px solid ${phStyle3?.border};border-radius:99px;padding:2px 8px;margin-left:4px" title="Fase ${invPhase}${phaseCertain ? ' (zeker)' : ' (voorlopig)'}"><span style="font-size:0.65rem;color:${phStyle3?.text};font-weight:600">� ${invPhase}${phaseCertain ? " " : " ~"+Math.round(phaseConf*100)+"%"}</span></span>`)
      : "";
  ...
  static get styles() {
    return css`
      ...
      ha-card {
        ...
        border-radius: var(--c-r); overflow: visible;
      ...
      /* Content */
      .content { padding:14px 15px 18px; overflow:hidden; border-radius:0 0 var(--c-r) var(--c-r); }
