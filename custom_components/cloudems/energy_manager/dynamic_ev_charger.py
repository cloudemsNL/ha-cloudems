...
class DynamicEVCharger:
    ...
    # ── Prijsdrempel kalibratie ──────────────────────────────────────────────

    def feed_price_history(self, prices: list[float]) -> None:
        """
        Voed historische EPEX-uurprijzen (vanuit coordinator _price_hour_history).
        Berekent het p35-percentiel als nieuwe cheap_threshold.

        Aanroepen vanuit coordinator elke update-cyclus als er nieuwe prijzen zijn.
        """
        if not prices:
            return
        # Voeg nieuwe prijzen toe, houd max 720 uur (30 dagen)
        self._price_history.extend(p for p in prices if 0 < p < 2.0)
        if len(self._price_history) > 720:
            self._price_history = self._price_history[-720:]
        if len(self._price_history) < 48:
            return   # te weinig data voor kalibratie
        # p35-percentiel: 35% van de tijd is stroom goedkoper dan deze waarde
        sorted_p = sorted(self._price_history)
        idx = int(len(sorted_p) * 0.35)
        new_threshold = round(sorted_p[idx], 4)
        # Clamp: nooit meer dan 50% afwijken van de geconfigureerde drempel
        lo = self._config_threshold * 0.50
        hi = self._config_threshold * 1.80
        new_threshold = max(lo, min(hi, new_threshold))
        if abs(new_threshold - self._calibrated_threshold) > 0.002:
            _LOGGER.debug(
                "DynamicEVCharger: drempel bijgesteld %.4f → %.4f EUR/kWh "
                "(p35 van %d uur data)",
                self._calibrated_threshold, new_threshold, len(self._price_history),
            )
