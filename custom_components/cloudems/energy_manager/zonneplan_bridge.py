...
class DecisionResult:
    ...
    discharge_w:       Optional[float] = None  # v5.5.359: alias voor opportunistic discharge
...
_ENT_PATTERNS: dict[str, list[str]] = {
    ...
    # v5.5.364: all-in tarief (incl. EB + BTW) van ha-zonneplan-one
    # electricity_tariff = kale EPEX; import_tariff = all-in (Zonneplan app prijs)
    "electricity_tariff_all_in": ["current_electricity_import_tariff",
                                  "electricity_import_tariff",
                                  "tariff_all_in", "all_in_tariff"],
    ...
    # ── Energie vandaag (geladen/ontladen kWh) ────────────────────────────────
    # Niet standaard beschikbaar via Zonneplan HA integratie maar wel via
    # sommige andere batterij-integraties (Victron, SMA, Huawei Luna, etc.)
    # Als aanwezig: gebruikt als primaire kWh-bron (exact, geen berekening)
    "charged_today":        ["charged_today",           "energy_charged_today",
                             "charge_kwh_today",        "geladen_vandaag",
                             "charged_energy_today",    "battery_charged_kwh"],
    "discharged_today":     ["discharged_today",        "energy_discharged_today",
                             "discharge_kwh_today",     "ontladen_vandaag",
                             "discharged_energy_today", "battery_discharged_kwh"],
}
...
class ZonneplanProvider(BatteryProvider):
    ...
    def __init__(self, hass: HomeAssistant, config: dict) -> None:
        ...
        self._charge_w:       float  = float(config.get("zonneplan_charge_w", 10000.0))
        self._discharge_w:    float  = float(config.get("zonneplan_discharge_w", 10000.0))
        # Anti-rondpompen
        ...
        # v5.5.363: geleerde price_floor op basis van EPEX historiek
        self._epex_samples:     list  = []   # laatste 200 prijzen voor percentiel-berekening
        self._learned_floor:    float = 0.05 # gestart als default
        # Idempotentie: cache van last gestuurde waarden
        ...
        self._cur_plan_deliver_w:  float = 0.0   # v5.5.465: bedoelde planwaarde
        self._cur_plan_charge_w:   float = 0.0
        self._last_slider_reason:  str            = ""
        self._seasonal_adjusted:   bool           = False
        ...
        self._ctx_congestion_active:   bool  = False  # netcongestie active
        ...
        self._last_decision_ts:    Optional[float] = None   # time.time() van last decide_action_v3
        self._last_decision_result: Optional[object] = None  # last DecisionResult
        ...
        self._surplus_high_since:  float = 0.0     # timestamp first keer surplus > drempel
        self._last_slider_write:   float = 0.0     # timestamp last slider/mode schrijfactie
        self._slider_min_interval:  float = 120.0   # v5.5.465: min seconden tussen slider writes
        self._slider_backoff_until: float = 0.0     # v5.5.465: back-off tot deze ts na bounce
        ...
        # cycli zonder data. Voorkomt valse melding bij temporary unavailable entity.
    ...
    async def _async_startup_send(self) -> None:
        ...
        # Wait 10s zodat HA entities geladen zijn
    ...
    def update_context(
        ...
        self._last_known_solar_w        = max(0.0, solar_now_w)
        self._last_pv_forecast_today_kwh= max(0.0, pv_forecast_today_kwh)

    def _get_effective_price(self) -> float:
        """All-in prijs in €/kWh (incl. energiebelasting, BTW, leveranciersopslag).

        Prioriteit:
        1. price_info.current_all_in (EPEX + belasting pad — werkt goed)
        2. raw.electricity_tariff_all_in_eur (ha-zonneplan-one all-in sensor)
        3. Fallback: kale EPEX + geconfigureerde belasting (enkel als niets anders)

        Fix #47 (Roedie): ha-zonneplan-one biedt current_electricity_import_tariff
        die all-in is (= wat de Zonneplan app toont). De kale electricity_tariff
        is EPEX-only en werd ten onrechte als all-in behandeld.
        """
        all_in = self._price_info.get("current_all_in")
        if all_in is not None:
            return float(all_in)
        # v5.5.364: all-in sensor van ha-zonneplan-one (current_electricity_import_tariff)
        tariff_all_in = self._last_state.raw.get("electricity_tariff_all_in_eur")
        if tariff_all_in is not None and float(tariff_all_in) > 0.01:
            return float(tariff_all_in)
    ...
    def _apply_seasonal_adaptation(self) -> None:
        """Seizoensadaptatie: winter hogere min-SOC, zomer meer PV-ruimte.

        Winter (nov-feb): min_soc +5% (backup voor koud weer, minder PV)
        Zomer (apr-sep):  max_soc -5% als morgen veel zon verwacht (ruimte voor PV)
        """
        import datetime as _dt_sea
        month = _dt_sea.datetime.now().month
        base_min = float(self._config.get("battery_min_soc", 10.0))
        base_max = float(self._config.get("battery_max_soc", 95.0))

        if month in (11, 12, 1, 2):   # winter
            self._min_soc = min(base_min + 5.0, 25.0)  # max 25% minimum
        elif month in (4, 5, 6, 7, 8, 9):  # zomer
            self._min_soc = base_min
            # Controleer of morgen veel PV verwacht → maak ruimte
            _pv_tom = getattr(self, '_last_pv_forecast_tomorrow_kwh', 0.0) or 0.0
            if _pv_tom > 15:  # >15 kWh morgen → laat 5% extra ruimte
                self._max_soc = max(base_max - 5.0, 80.0)
            else:
                self._max_soc = base_max
        else:  # overgangsmaanden
            self._min_soc = base_min
            self._max_soc = base_max

    @property
    def _effective_charge_w(self) -> float:
        """Max laadvermogen — geconfigureerd (default 10000W = entity max).
        Urgentie-scaling geeft asymmetrische waarden per cyclus.
        """
        return min(self._charge_w, self._slider_max_solar_w)

    @property
    def _effective_discharge_w(self) -> float:
        """Max ontlaadvermogen — geconfigureerd (default 10000W = entity max)."""
    ...
    async def async_detect(self) -> bool:
        try:
            ...
            # First pass: doorzoek ALLE zonneplan_one platform entities (incl. tarieven, forecast)
            ...
            # op het zonneplan_one platform, ook als naam niet matcht op known patronen.
            ...
            # Succes als soc OF control_mode found (minimaal batterij aanwezig)
    ...
    def read_state(self) -> BatteryProviderState:
        ...
        # v4.6.136: hysterese — alleen 'offline' na 3 opeennext cycli zonder data.
        # Voorkomt valse offline-melding bij temporary unavailable entity (split-second).
        ...
        self._last_state = BatteryProviderState(
            ...
            raw = {
                ...
                # Gebruik "unknown" als geen tariefgroep found — niet "normal"
                ...
                # v5.5.364: all-in tarief van ha-zonneplan-one
                "electricity_tariff_all_in_eur": _f("electricity_tariff_all_in"),
    ...
    def decide_action_v3(
        ...
        # Seizoensadaptatie: pas SOC grenzen aan op seizoen
        self._apply_seasonal_adaptation()
        """
        ...
        # Cache last known tariefgroep — voorkomt "unknown" direkt na herstart
        ...
        # v5.5.363: leer price_floor van historische prijzen
        if price > 0:
            self._epex_samples.append(price)
            if len(self._epex_samples) > 200: self._epex_samples.pop(0)
        if len(self._epex_samples) >= 48:  # minstens 48 uur data
            # Floor = 15e percentiel — goedkoopste uren maar niet extreem outliers
            _sorted = sorted(self._epex_samples)
            _p15 = _sorted[int(len(_sorted) * 0.15)]
            # Houd rekening met saldering: bij hoge saldering is floor hoger (eigenverbruik waardevoller)
            _nm_factor = 1.0 + (float(getattr(self, '_last_net_metering_pct', 36)) / 100) * 0.3
            self._learned_floor = max(0.02, min(0.15, _p15 * _nm_factor))
        # Gebruik geleerde floor als die lager is dan geconfigureerde (meer conservatief)
        _effective_floor = min(self._price_floor, self._learned_floor) if len(self._epex_samples) >= 48 else self._price_floor
        ...
        # v5.5.167: Stroomonderbreking reserve — altijd apart houden
        _blackout_reserve = float(getattr(self, '_blackout_reserve_pct', 0) or 0)
        if _blackout_reserve > 0:
            min_soc = max(min_soc, _blackout_reserve)
            _LOGGER.debug(
                "ZP: blackout reserve %.0f%% — min_soc verhoogd van %.0f%% naar %.0f%%",
                _blackout_reserve, self._min_soc, min_soc
            )
        ...
        # Store absorptieruimte als PV vandaag nog kWh levert
        ...
        # ── Saldering-context ──────────────────────────────────────────────────
        # Get huidig salderingspercentage op (automatisch op basis van jaar).
        ...
        # Calculate minimale verkoopprijs voor winstgevend ontladen.
        ...
        _sal_min_discharge_price = sal_ctx.min_discharge_price_for_profit(
            buy_price_eur_kwh   = max(0.0, price),   # current prijs als proxy voor laadkosten
        ...
        # ══════════════════════════════════════════════════════════════════════
        # CENTRALE PV-SURPLUS BESCHERMING
        # Als er vandaag veel zon verwacht wordt (> 80% van batterijcapaciteit)
        # dan heeft ontladen geen zin — de accu wordt toch gratis gevuld.
        # Enige uitzondering: prijs is extreem hoog (> 2x gemiddeld).
        # ══════════════════════════════════════════════════════════════════════
        _pv_rich_day = (
            cap_kwh > 0
            and pv.forecast_today_kwh > cap_kwh * 0.80
        )
        _price_very_high = (
            price > 0
            and len(self._epex_samples) >= 12
            and price > sorted(self._epex_samples)[int(len(self._epex_samples) * 0.80)] * 1.5
        )
        _discharge_blocked_by_pv = _pv_rich_day and not _price_very_high
        ...
        if price < 0.0:
            ...
            return DecisionResult(action=ZPAction.HOLD, confidence=1.0, reasons=reasons,
                                  ...
                                  human_reason=f"Stroom is gratis maar batterij staat al vol op {soc:.0f}%. Zonneplan beheert op standaard instellingen.")

        # ══════════════════════════════════════════════════════════════════════
        # PRIORITEIT 2 — PRIJS ONDER FLOOR (niet HIGH)
        # ══════════════════════════════════════════════════════════════════════
        if price < (self._learned_floor if len(self._epex_samples) >= 48 else self._price_floor) and tg != "high":
            reasons.append(
            ...
            return DecisionResult(action=ZPAction.HOLD, confidence=0.8, reasons=reasons,
                                  ...
                                  human_reason=f"Stroomprijs is laag (€{price:.4f}/kWh). Geen bijzondere actie — Zonneplan beheert de batterij op standaard instellingen.")
        ...
        # Schat netto huis-eigenverbruik (als ML forecast available, anders 800W default)
        ...
        if tg == "high":
            ...
            # If HIGH-tarief onder de minimale winstgevende prijs valt, is
            # ontladen (deels) naar het net niet rendabel.
            # Uitzondering: als het huis veel stroom verbruikt (eigenverbruik > 80%)
            # is ontladen always zinvol — we besparen de volle importprijs.
            ...
            if n_future_high == 0:
                # Last HIGH-blok — ontlaad naar PV-gecorrigeerde bodem
                ...
                if soc > soc_target and not _discharge_blocked_by_pv:
        ...
        # ══════════════════════════════════════════════════════════════════════
            ...
            # Schat discharge prijs als 2× current LOW prijs (HIGH is typisch 2-3× LOW)
        ...
        # ══════════════════════════════════════════════════════════════════════
            ...
            if first_low_h is not None and first_low_h <= 2:
                ...
                if headroom < 15 and soc > min_soc + 10 and not _discharge_blocked_by_pv:
                    reasons.append(
            ...
            # If de batterij (bijna) vol is EN er zijn morgen/komende uren veel PV
            ...
            #   4. Ontladen is rendabel bij current prijs (saldering-check)
            _high_pv_expected = (
            ...
            if (_battery_nearly_full and _high_pv_expected and
                ...
                if _gap > 5.0 and not _discharge_blocked_by_pv:
            ...
            # v5.5.346: bij NORMAL tarief + hoog grid + hoog SoC → ontlaad
            # Log-analyse toonde nacht SoC=97%, grid=3kW+ maar hold → verloren geld
            _grid_norm = float(getattr(self, '_last_grid_w', 0) or 0)
            # v5.5.363: geleerde opportunistic drempel op basis van huis basisverbruik
            # Als grid > 1.5× het gemiddelde basisverbruik → significant genoeg om te ontladen
            _base_load_opp = float(getattr(self, '_learned_base_load_w', 2500))
            _opp_threshold = max(1500.0, min(5000.0, _base_load_opp * 1.5))
            if (not _discharge_blocked_by_pv
                    and _grid_norm > _opp_threshold
                    and soc > (min_soc + 20)
                    and soc > 60):
                _dis_w = min(self._effective_discharge_w, _grid_norm * 0.9)
                reasons.append(
                    f"NORMAL, grid={_grid_norm:.0f}W > drempel {_opp_threshold:.0f}W + SoC={soc:.0f}% → ontlaad"
                )
                return DecisionResult(
                    action=ZPAction.DISCHARGE, confidence=0.7,
                    reasons=reasons, soc_target=soc_target, soc_reachable=soc,
                    discharge_w=round(_dis_w),
                    human_reason=(
                        f"Normaal tarief. Het net levert {_grid_norm:.0f}W terwijl de "
                        f"batterij op {soc:.0f}% staat — even ontladen bespaart kosten."
                    ),
                )
            ...
            return DecisionResult(action=ZPAction.HOLD, confidence=0.6,
                                  reasons=reasons, soc_target=soc_target,
                                  soc_reachable=soc,
                                  human_reason="Normaal tarief — Zonneplan beheert zelf op basis van beschikbare PV en huisverbruik.")
        ...
        return DecisionResult(action=ZPAction.HOLD, confidence=0.5,
                              reasons=reasons, soc_target=soc_target,
                              soc_reachable=soc,
                              human_reason="Situatie onbekend — Zonneplan beheert op standaard instellingen.")

    # ── Backwards-compat alias ─────────────────────────────────────────────────

    def set_blackout_reserve(self, pct: float) -> None:
        """Stel minimale SoC in voor stroomonderbreking reserve (0=uit)."""
        self._blackout_reserve_pct = max(0.0, min(50.0, pct))
        _LOGGER.info("ZonneplanProvider: blackout reserve ingesteld op %.0f%%", self._blackout_reserve_pct)
    ...
    def get_forecast_summary(self) -> dict:
        ...
        # Save voor tijdstempel weergave in dashboard
        import time as _t
    ...
    async def async_set_discharge(self, power_w: Optional[float] = None,
        ...
        if soc is not None and soc <= self._min_soc:
            _LOGGER.debug("ZonneplanProvider: ontladen overgeslagen (SoC %.0f%% <= min %.0f%%)",
                          soc, self._min_soc)
            return False
        # EV-blokker always (ook bij HIGH)
    ...
    async def async_apply_forecast_decision_v3(
        ...
        soh_pct:                 float = 100.0,
        net_metering_pct:        float = 1.0,
        grid_w:                  float = 0.0,   # v5.5.357: grid voor opportunistic discharge
        plan_deliver_w:          float = 0.0,   # v5.5.465: geplande discharge uit plan
        plan_charge_w:           float = 0.0,   # v5.5.465: geplande charge uit plan
    ) -> DecisionResult:
        """
        ...
        """
        # v5.5.465: sla huidige planwaarden op voor kaart display
        self._cur_plan_deliver_w = plan_deliver_w
        self._cur_plan_charge_w  = plan_charge_w
        ...
        # Detect of gebruiker BUITEN CloudEMS om de modus heeft gewijzigd
        # If active_mode verschilt van last_sent_mode → gebruiker heeft manueel ingegrepen
        ...
        if (_expected and _active and _active != _expected
            ...
            # Default override duur: 30 minuten
            _override_min = 30
        ...
        # v5.5.346: sla grid_w op zodat decide_action_v3 opportunistic discharge kan doen
        ...
        _LOGGER.info(
            ...
            " | ".join(result.reasons[:2]),   # first 2 redenen in log
        )

        if result.action == ZPAction.CHARGE:
            _chg_w = plan_charge_w if plan_charge_w > 0 else (result.charge_power_w or self._effective_charge_w)
            result.executed = f"charge ({_chg_w:.0f}W)"
            ...
            await self.async_set_charge(power_w=_chg_w)
            # Zorg dat deliver slider ook op standaard staat
            _del_default = plan_deliver_w if plan_deliver_w >= 0 else self._effective_discharge_w
            await self._set_slider_idempotent("deliver_to_home", _del_default, self._last_sent_deliver_w)
            self._last_sent_deliver_w = _del_default
            self._last_slider_reason = (
                f"Laden {_chg_w:.0f}W · Levering {_del_default:.0f}W (standaard)"
            )

        elif result.action == ZPAction.DISCHARGE:
            # v5.5.465: gebruik plan_deliver_w als slider-waarde, bridge bepaalt alleen richting
            _dis_w = plan_deliver_w if plan_deliver_w >= 0 else (result.discharge_power_w or self._effective_discharge_w)
            await self.async_set_discharge(
                power_w=_dis_w,
                bypass_antiround=result.bypass_antiround,
            )
            result.executed = f"discharge ({_dis_w:.0f}W)"
            self._last_expected_action = "discharge"
            self._last_action_ts = __import__("time").time()
            self._last_battery_w_at_cmd = float(self._last_state.power_w or 0)
            # v5.5.465: solar slider = plan_charge_w (0 tijdens ontladen)
            _sol_default = plan_charge_w if plan_charge_w >= 0 else self._effective_charge_w
            await self._set_slider_idempotent("solar_charge", _sol_default, self._last_sent_solar_w)
            self._last_sent_solar_w = _sol_default
            self._last_slider_reason = (
                f"Levering {_dis_w:.0f}W (prijs-optimaal) · Solar {_sol_default:.0f}W (standaard)"
            )

        elif result.action == ZPAction.HOLD:
            # HOLD = home_opt met sliders op geconfigureerde waarden
            # Nexus levert altijd aan huis — er is geen echte "hold"
            ...
            # HOLD: sliders op hardware max — Zonneplan begrenst zelf op zijn limieten
            _deliver_target = plan_deliver_w if plan_deliver_w >= 0 else self._effective_discharge_w  # v5.5.465: plan bepaalt (>=0 zodat 0W ook geldig is)
            _solar_target   = plan_charge_w if plan_charge_w >= 0 else self._effective_charge_w  # v5.5.465: plan
            if self._entities.get("deliver_to_home"):
                await self._set_slider_idempotent(
                    "deliver_to_home", _deliver_target, self._last_sent_deliver_w
                )
                self._last_sent_deliver_w = float(_deliver_target)
            if self._entities.get("solar_charge"):
                await self._set_slider_idempotent(
                    "solar_charge", _solar_target, self._last_sent_solar_w
                )
                self._last_sent_solar_w = float(_solar_target)

            # Bouw uitleg waarom deze slider-waarden
            _why_deliver = f"standaard {_deliver_target:.0f}W"
            _why_solar   = f"standaard {_solar_target:.0f}W"
            # Pas reden aan op context
            soc_now = self._last_state.soc_pct or 0
            if soc_now >= self._max_soc:
                _why_solar = f"batterij vol ({soc_now:.0f}%) → max laden"
            elif soc_now <= self._min_soc + 5:
                _why_deliver = f"bijna leeg ({soc_now:.0f}%) → min levering"
            # Toon clamped waarde (wat daadwerkelijk gestuurd wordt)
            _clamped_del = min(round(_deliver_target, 0), self._slider_max_deliver_w)
            _clamped_sol = min(round(_solar_target, 0), self._slider_max_solar_w)
            self._last_slider_reason = (
                f"Levering {_clamped_del:.0f}W ({_why_deliver}) · "
                f"Solar {_clamped_sol:.0f}W ({_why_solar})"
            )
            result.executed = (
                f"home_opt lever={_deliver_target:.0f}W solar={_solar_target:.0f}W"
            )
        ...
        if self._entities.get("deliver_to_home") and self._entities.get("control_mode"):
            ...
            if _surplus_high_condition:
                ...
                # Activate pas na 60s aanhoudend surplus (debounce)
            ...
            if _surplus_low_condition and self._surplus_slider_high:
                ...
                # Deactivate pas na 120s geen surplus (debounce terugzetten)
    ...
    async def async_force_slider_calibrate(self) -> dict:
        ...
        # Gebruik geconfigureerde waarden (niet begrensd op learned max)
    ...
    def get_setup_warnings(self) -> list[dict]:
        ...
        if power_eid:
            try:
                ...
                pass
        # Check: tariff_group niet found maar provider wel active
    ...
    def get_info(self) -> dict:
        ...
        base.update({
            ...
            "charged_today_kwh":     _fv("charged_today"),
            "discharged_today_kwh":  _fv("discharged_today"),
        ...
        base["plan_deliver_w"] = float(self._cur_plan_deliver_w)
        base["plan_charge_w"]  = float(self._cur_plan_charge_w)
        base["slider_queue"]   = self._cmd_queue.get_diagnostics() if getattr(self,"_cmd_queue",None) else {}
        return base
    ...
    async def _send_control_mode(self, mode_val: str) -> bool:
        ...
        # Idempotentie: check current staat
        ...
        if current == mode_val and self._last_sent_mode == mode_val:
            if self._startup_send_done:
                ...
                return True
            # Na herstart always één keer sturen, ook als modus al klopt
    ...
    async def _set_slider_idempotent(self, key: str, value_w: float,
        ...
        # v5.5.465: rate limit — min interval + back-off na bounce
        _now = time.time()
        if _now < self._slider_backoff_until:
            _LOGGER.debug("ZonneplanProvider: slider %s in back-off (%.0fs resterend)", key,
                          self._slider_backoff_until - _now)
            return
        if (_now - self._last_slider_write) < self._slider_min_interval:
            _LOGGER.debug("ZonneplanProvider: slider %s te vroeg (%.0fs < min %.0fs)", key,
                          _now - self._last_slider_write, self._slider_min_interval)
            return

        # Begrens op learned maximum
        ...
        # Vergelijk ook met werkelijke HA state — Zonneplan cloud kan slider resetten
        _actual_now = None
        try:
            _st = self._hass.states.get(eid)
            if _st and _st.state not in ("unavailable", "unknown"):
                _actual_now = round(float(_st.state), 0)
        except Exception:
            pass

        if last_sent == clamped_w and (_actual_now is None or abs(_actual_now - clamped_w) <= 50):
            _LOGGER.debug("ZonneplanProvider: slider %s al %.0fW (actual=%.0fW), geen call",
                          key, clamped_w, _actual_now or 0)
            return

        if _actual_now is not None and abs(_actual_now - clamped_w) > 50:
            _LOGGER.info(
                "ZonneplanProvider: slider %s actual=%.0fW maar verwacht=%.0fW — corrigeren",
                key, _actual_now, clamped_w
            )
        ...
        try:
            ...
            _LOGGER.info(
                "ZonneplanProvider: slider %s → %.0fW",
                key, clamped_w,
            )
        except Exception as exc:
        ...
        # Read na 30s de werkelijke staat terug — geen extra cloud-call,
        # HA heeft de Zonneplan-entiteit al geupdated via polling.
        sent_w = clamped_w

        async def _readback(_now) -> None:
            # Never learned_max aanpassen tijdens een actieve probe — probe doet dit zelf
            if self._probe_active:
                return
            st = self._hass.states.get(eid)
            if not st or st.state in ("unavailable", "unknown"):
            ...
            # Lees HA entity max attribuut als absolute bovengrens
            entity_max = old_max
            try:
                _st2 = self._hass.states.get(eid)
                if _st2:
                    entity_max = float(_st2.attributes.get("max", old_max))
            except Exception:
                pass

            if actual_w < sent_w - 50:
                # Nexus heeft afgeknepen of 429 — back-off verdubbelen (max 30 min)
                new_max = actual_w
                self._slider_backoff_until = time.time() + min(1800, self._slider_min_interval * 2)
                self._slider_min_interval  = min(600, self._slider_min_interval * 1.5)
                _LOGGER.warning(
                    "ZonneplanProvider: slider %s bounce %.0fW→%.0fW — "
                    "back-off %.0fs, nieuw interval %.0fs",
                    key, sent_w, actual_w,
                    self._slider_backoff_until - time.time(), self._slider_min_interval,
                )
                new_max = actual_w
            else:
                # Zonneplan accepteerde — interval langzaam terugbrengen naar minimum
                new_max = entity_max
                self._slider_min_interval = max(120.0, self._slider_min_interval * 0.9)
                _LOGGER.debug(
                    "ZonneplanProvider: slider %s geaccepteerd %.0fW — interval %.0fs",
                    key, sent_w, self._slider_min_interval,
                )
