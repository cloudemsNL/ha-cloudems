...
class GridCongestionDetector:
    ...
    async def async_evaluate(
        ...
        if self._active or utilisation > 90:
            actions.append({
                "action":   "reduce_ev_charging",
                "priority": 1,
                "reason":   f"Grid at {utilisation:.0f}% — reduce EV to minimum",
            })
        if self._active and price_high:
            actions.append({
                "action":   "defer_boiler",
                "priority": 2,
                "reason":   f"Grid congestion + high price ({price_eur_kwh:.3f} €/kWh) — pause boiler",
            })
