...
# kan het fysiek niet op 1 fase zitten (20A × 230V = 4600W veiligheidsmarge).
# NL standaard hoofdzekering is 3×25A; zelfs met 3×80A aansluiting geldt dit.
SINGLE_PHASE_MAX_W = 4600.0  # W — boven deze waarde = zeker 3-fasig
...
class SolarPowerLearner:
    ...
    def __init__(self, hass: HomeAssistant, inverter_configs: list[dict],
                 fuse_a: float = 25.0) -> None:
        self.hass = hass
        ...
        # Geconfigureerde netcapaciteit per fase — voor peak-power 3F heuristiek
        self._fuse_a: float = float(fuse_a)
        # Drempel: 1 fase kan nooit meer leveren dan fuse_a × 230V
        # Gebruik 95% van theoretisch max als drempel (5% meettolerantie)
        self._single_phase_max_w: float = self._fuse_a * 230.0 * 0.95
    ...
    async def async_setup(self, backup: Optional[Any] = None) -> None:
        ...
        for cfg in self._configs:
            eid   = cfg["entity_id"]
            # Label bepalen: config label > HA friendly_name > entity_id suffix
            _raw_label = cfg.get("label", "")
            if _raw_label and _raw_label != eid:
                label = _raw_label
            else:
                # Probeer friendly_name uit HA state
                _st = self.hass.states.get(eid)
                if _st and _st.attributes.get("friendly_name"):
                    label = _st.attributes["friendly_name"]
                else:
                    # Fallback: entity_id suffix zonder domein, underscores → spaties
                    label = eid.split(".")[-1].replace("_", " ").title()
            d     = saved.get(eid, {})
            ...
            # Herstel aaneengesloten-seconden teller voor net-capaciteit heuristiek
            profile._peak_over_single_phase_s = float(d.get("peak_over_1f_s", 0.0))
    ...
    async def async_update(self, phase_currents: dict[str, float] | None = None) -> None:
        ...
        for eid, profile in self._profiles.items():
            ...
            # ── Net-capaciteit heuristiek: 3-fase als output > 1-fase maximum ──
            # "Langere tijd boven max" = accumuleer seconden boven drempel.
            # 10s update-interval × 6 samples = 60 seconden aaneengesloten.
            # Dit voorkomt dat een korte meetpiek (spike) direct triggert.
            _OVER_SECONDS_NEEDED = 60.0   # aaneengesloten seconden boven 1F-max
            _TICK_S              = 10.0   # coordinator update-interval
            if (not profile.is_three_phase
                    and power_w > self._single_phase_max_w):
                profile._peak_over_single_phase_s = getattr(
                    profile, "_peak_over_single_phase_s", 0.0) + _TICK_S
                if profile._peak_over_single_phase_s >= _OVER_SECONDS_NEEDED:
                    _LOGGER.warning(
                        "SolarLearner [%s]: %.0fs boven 1-fase max (%.0fW > %.0fW "
                        "@ %.0fA net) — omvormer is zeker 3-fasig.",
                        profile.label,
                        profile._peak_over_single_phase_s,
                        power_w, self._single_phase_max_w, self._fuse_a,
                    )
                    profile.is_three_phase            = True
                    profile.phase_certain             = True
                    profile.three_phase_votes         = THREE_PHASE_MIN_DETECTIONS
                    profile._peak_over_single_phase_s = 0.0
                    profile.phase_votes               = {}
                    self._dirty = True
            else:
                # Vermogen weer onder drempel — reset teller (niet aaneengesloten)
                profile._peak_over_single_phase_s = 0.0

            # Herstel: al 3F via heuristiek maar phase_certain gereset?
            if (profile.is_three_phase
                    and not profile.phase_certain
                    and profile.peak_power_w > self._single_phase_max_w):
                _LOGGER.info(
                    "SolarLearner [%s]: phase_certain hersteld via net-capaciteit "
                    "(piek %.0fW > 1F-max %.0fW).",
                    profile.label, profile.peak_power_w, self._single_phase_max_w,
                )
                profile.phase_certain = True
                self._dirty = True
    ...
    async def _detect_phase(self, profile: InverterProfile, power_w: float, phase_currents: dict) -> None:
        ...
        # ── 3-fase check (v2.6 rewrite) ───────────────────────────────────────
        # Strategie: kijk of MEERDERE fasen significant reageren op de vermogenssprong.
        # We gebruiken GEEN vaste balance-ratio meer — ZZO omvormers met ongelijke
        # strings (bijv. Growatt 5kWp oost + Goodwe 2kWp west) verdelen NIET gelijk.
        #
        # Criterium: minstens 2 van 3 fasen stijgen met > drempel_per_fase.
        # Drempel per fase = power_delta / (3 × 230) met minimum van 0.06A.
        # Eén fase mag negatief/nul zijn door meetruis of ongelijke string-verdeling.
        if len(phases_available) == 3:
            per_phase_threshold = max(0.06, power_delta / 690.0)
            pos_significant = [p for p in phases_available if deltas[p] >= per_phase_threshold]
            any_negative     = any(deltas[p] < -0.3 for p in phases_available)

            if len(pos_significant) >= 2 and not any_negative:
                # ✅ 3F patroon: minstens 2 fasen reageren, geen fase hard negatief
                profile.three_phase_votes += 1
                self._dirty = True

                # Onterecht als enkelfasig gecached? Reset zodat 3F kan winnen.
                if profile.phase_certain and not profile.is_three_phase:
                    _LOGGER.info(
                        "SolarLearner '%s': 3F-stem terwijl single-fase (%s) gecached — reset detectie",
                ...
                sym_pct = (min(deltas[p] for p in pos_significant) /
                           max(deltas[p] for p in pos_significant) * 100) if pos_significant else 0
                _LOGGER.info(
                    "SolarLearner '%s': 3F-stem %d/%d — deltas L1=%.2f L2=%.2f L3=%.2f "
                    "(>%.2fA: %d fasen, sym≈%.0f%%) ΔPV=+%.0fW",
                    profile.label,
                    profile.three_phase_votes, THREE_PHASE_MIN_DETECTIONS,
                    deltas.get("L1", 0), deltas.get("L2", 0), deltas.get("L3", 0),
                    per_phase_threshold, len(pos_significant), sym_pct, power_delta,
                )
                if profile.three_phase_votes >= THREE_PHASE_MIN_DETECTIONS:
                    _LOGGER.warning(
                        "SolarLearner '%s': ✅ 3-FASIG BEVESTIGD na %d stemmen",
                    ...
                    self._dirty = True
                return  # niet doorgaan naar enkelfase-check

        # ── Enkelfase check ───────────────────────────────────────────────────
        # Sla enkelfase-stem ALTIJD over als alle 3 fasen positief bewegen —
        # dat patroon is 3-fasig, ook als de balance-ratio net niet gehaald werd.
        # Zo kan een noisy 3F-omvormer nooit ten onrechte als enkelfasig worden gecached.
        if len(phases_available) == 3:
            all_positive = all(deltas[p] > -0.15 for p in phases_available)
            if all_positive:
                return  # 3F patroon — geen enkelfase-stem
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            eid: {
                ...
                "peak_over_1f_s":      getattr(p, "_peak_over_single_phase_s", 0.0),
