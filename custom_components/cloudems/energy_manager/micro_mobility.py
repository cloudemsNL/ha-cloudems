...
class MicroMobilityTracker:
    ...
    _NON_CHARGER_STARTUP_KEYWORDS = (
        ...
        "boiler", "warmtepomp",
        "piano", "keyboard", "guitar", "gitaar", "synthesizer",
        "projector", "beamer", "monitor", "display", "screen",
        "airco", "airconditioning", "ventilator", "fan",
        "pomp", "pump", "motor", "drill", "boor",
        "treadmill", "loopband", "fitness",
        "game", "console", "playstation", "xbox",
        "koffie", "coffee", "espresso", "blender", "mixer",
        "stofzuiger", "vacuum", "robot",
        "air purifier",
    )
    ...
    async def async_setup(self) -> None:
        ...
        for sd in saved.get("sessions", []):
            try:
                sess = MicroEVSession.from_dict(sd)
                # Prune opgeslagen sessies van niet-voertuig apparaten
                label_low = (sess.label or "").lower()
                if not any(kw in label_low for kw in self._NON_CHARGER_STARTUP_KEYWORDS):
                    self._sessions.append(sess)
            except Exception:
                pass
        self._rebuild_profiles()
        # Herstel handmatig bevestigde profielen uit opgeslagen staat
        # (label-overrides en vehicle_type bevestigingen overleven zo een herstart)
        for did, pd in saved.get("profiles", {}).items():
            if did in self._profiles:
                p = self._profiles[did]
                # Alleen overschrijven als de opgeslagen versie een bevestigd profiel is
                if pd.get("confirmed"):
                    p.confirmed    = True
                    p.label        = pd.get("label", p.label)
                    p.avg_power_w  = float(pd.get("avg_power_w", p.avg_power_w))
                    _LOGGER.debug("MicroMobility: profiel hersteld: %s (%s)", p.label, did)
        # Verwijder ook profielen van niet-voertuigen uit het profiel-woordenboek
        to_remove = [
            did for did, p in self._profiles.items()
            if any(kw in (p.label or "").lower() for kw in self._NON_CHARGER_STARTUP_KEYWORDS)
        ]
        for did in to_remove:
            del self._profiles[did]
            _LOGGER.info("MicroMobility: niet-voertuig profiel verwijderd bij opstart: %s", did)
        _LOGGER.info(
    ...
    def update(self, nilm_devices: list[dict], price_eur_kwh: float = 0.0) -> None:
        ...
        for dev in nilm_devices:
            ...
            _NON_CHARGER_NAME_KEYWORDS = (
                ...
                "oven", "magnetron", "microwave", "kettle", "waterkoker",
                "boiler", "warmtepomp",
                # Muziekinstrumenten, entertainment, overige niet-laders
                "piano", "keyboard", "guitar", "gitaar", "synthesizer",
                "projector", "beamer", "monitor", "display", "screen",
                "airco", "airconditioning", "ventilator", "fan",
                "pomp", "pump", "motor", "drill", "boor",
    ...
    def get_data(self) -> MicroMobilityData:
        ...
        # Voertuigprofielen — filter opgeslagen niet-voertuig profielen uit
        _NON_CHARGER_KEYWORDS = (
            "purifier", "luchtreiniger", "heater", "verwarming", "lamp",
            "light", "koelkast", "fridge", "freezer", "vriezer",
            "tv", "television", "audio", "speaker", "printer",
            "router", "modem", "nas", "server", "computer",
        ...
        profile_list = [
            ...
            if not any(kw in p.label.lower() for kw in _NON_CHARGER_KEYWORDS)
        ]
    ...
    async def async_maybe_save(self) -> None:
        if self._dirty and (time.time() - self._last_save) >= SAVE_INTERVAL_S:
            # Persisteer ook profielen zodat hernoemde/bevestigde voertuigen
            # een HA-herstart overleven (anders gaan type-overrides verloren).
            profiles_dict = {}
            for did, p in self._profiles.items():
                try:
                    profiles_dict[did] = {
                        ...
                        "label":        p.label,
                        "vehicle_type": p.vehicle_type.value if hasattr(p.vehicle_type, "value") else str(p.vehicle_type),
            ...
            await self._store.async_save({
                "sessions": [s.to_dict() for s in self._sessions[-500:]],
