...
# - Baseline nog niet bevroren → 8 uur (apparaat nooit stabiel gezien)
# - Baseline bevroren, geen drift → 72 uur (actief apparaat, bewaren)
DRIFT_RETENTION_ALERT_H    =  4   # uur bewaren na drift-alert
DRIFT_RETENTION_LEARNING_H =  8   # uur bewaren als baseline nog niet klaar
DRIFT_RETENTION_NORMAL_H   = 72   # uur bewaren voor gezonde profielen
...
# Minimum absolute verschil in Watt voor een drift-alert — voorkomt valse alarmen bij
# kleine baseline-waarden (bijv. omvormer geleerd 's nachts op 2W → dag op 3000W = 150000%)
MIN_ABSOLUTE_DRIFT_W = 50.0   # Minstens 50W verschil voordat een drift-alert geldt

# Apparaattypen die nooit een drift-alert genereren (variabel van aard)
DRIFT_SKIP_TYPES = {
    ...
    "ev_charger",  # EV-lader — wisselend laden
}

# Label/entiteit-fragmenten die altijd overgeslagen worden — ter bescherming
# wanneer NILM een verkeerd device_type heeft toegewezen aan een PV/batterij sensor.
