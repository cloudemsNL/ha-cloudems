...
# Weekdag-gewichten — apparaten die vaker in het weekend of doordeweeks lopen
# Formaat: device_type → {"weekday": factor, "weekend": factor}
# 0=ma..4=vr = weekday, 5=za..6=zo = weekend
WEEKDAY_WEIGHTS: Dict[str, Dict[str, float]] = {
    "washing_machine": {"weekday": 0.85, "weekend": 1.35},  # weekend-was
    "dishwasher":      {"weekday": 1.05, "weekend": 0.95},  # iets minder weekend
    "oven":            {"weekday": 0.90, "weekend": 1.25},  # weekend koken
    "dryer":           {"weekday": 0.85, "weekend": 1.30},  # weekend samen met was
    "ev_charger":      {"weekday": 1.10, "weekend": 0.85},  # doordeweeks woon-werk
    "computer":        {"weekday": 1.15, "weekend": 0.80},  # werkcomputer
    "entertainment":   {"weekday": 0.90, "weekend": 1.20},  # weekend TV/games
    "heat_pump":       {"weekday": 1.00, "weekend": 1.10},  # weekend meer thuis
    "boiler":          {"weekday": 1.00, "weekend": 1.10},
}
...
class BayesianNILMClassifier:
    ...
    def __init__(self) -> None:
        ...
        self._last_temp_c: float = 15.0   # voor toegang vanuit NILMDetector
    ...
    def update_confidences(
        ...
        self._call_count += 1
        self._last_temp_c = temperature_c   # bewaar voor externe toegang
    ...
    def _compute_priors(
        ...
        for dt, base in BASE_PRIOR.items():
            ...
            wday_f   = (WEEKDAY_WEIGHTS.get(dt, {}).get("weekend", 1.0)
                        if is_weekend else
                        WEEKDAY_WEIGHTS.get(dt, {}).get("weekday", 1.0))

            p = (base + learned) * hourly * season_f * temp_f * wind_f * wday_f
