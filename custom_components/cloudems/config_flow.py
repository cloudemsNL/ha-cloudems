    ...
    async def async_step_inverter_detail(self, user_input=None):
        ...
        if user_input is not None:
            self._config[CONF_INVERTER_CONFIGS].append({
                ...
                "solar_dimmer":   bool(user_input.get("inv_solar_dimmer", False)),
            })
            self._inv_step += 1
            ...
            if self._config[CONF_INVERTER_CONFIGS]:
                ...
                # Derive global solar dimmer flag: enabled if ANY inverter has it on
                self._config[CONF_ENABLE_SOLAR_DIMMER] = any(
                    inv.get("solar_dimmer", False)
                    for inv in self._config[CONF_INVERTER_CONFIGS]
                )
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                # Zonnebegrenzing per omvormer: beperkt vermogen bij negatieve EPEX-prijzen
                vol.Optional("inv_solar_dimmer", default=bool(existing.get("solar_dimmer", False))): bool,
            }),
            description_placeholders={
