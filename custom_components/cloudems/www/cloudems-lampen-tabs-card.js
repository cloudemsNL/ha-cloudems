...
class CloudEMSLampenTabsCard extends HTMLElement {
  constructor() {
    ...
    this._setting = false;
    this._tabs = [
    {id:'lampen', label:'💡 Lampen', type:'custom:cloudems-lamp-card'},
    {id:'circadian', label:'🌅 Circadiaans', type:'custom:cloudems-circadian-card'},
    {id:'automaat', label:'🤖 Automaat', type:'custom:cloudems-lamp-auto-card'}
    ];
  }
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-lampen' data-tab='lampen'>💡 Lampen</div>
            <div class='tab' id='tab-circadian' data-tab='circadian'>🌅 Circadiaans</div>
            <div class='tab' id='tab-automaat' data-tab='automaat'>🤖 Automaat</div>
        </div>
            <div id='slot-lampen'></div>
            <div id='slot-circadian'></div>
            <div id='slot-automaat'></div>
      </div>`;
...
if (!window.customCards.find(c => c.type === 'cloudems-lampen-tabs-card'))
  window.customCards.push({ type: 'cloudems-lampen-tabs-card', name: 'CloudEMSLampenTabsCard' });
console.info(`%c CLOUDEMS-LAMPEN-TABS-CARD %c v${CLOUDEMS_LAMPEN_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
