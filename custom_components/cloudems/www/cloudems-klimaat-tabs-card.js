...
class CloudEMSKlimaatTabsCard extends HTMLElement {
  constructor() {
    ...
    this._setting = false;
    this._tabs = [
    {id:'airco', label:'❄️ Airco', type:'custom:cloudems-airco-card'},
    {id:'klimaat', label:'🌡 Klimaat', type:'custom:cloudems-klimaat-card'}
    ];
  }
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-airco' data-tab='airco'>❄️ Airco</div>
            <div class='tab' id='tab-klimaat' data-tab='klimaat'>🌡 Klimaat</div>
        </div>
            <div id='slot-airco'></div>
            <div id='slot-klimaat'></div>
      </div>`;
...
if (!window.customCards.find(c => c.type === 'cloudems-klimaat-tabs-card'))
  window.customCards.push({ type: 'cloudems-klimaat-tabs-card', name: 'CloudEMSKlimaatTabsCard' });
console.info(`%c CLOUDEMS-KLIMAAT-TABS-CARD %c v${CLOUDEMS_KLIMAAT_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
