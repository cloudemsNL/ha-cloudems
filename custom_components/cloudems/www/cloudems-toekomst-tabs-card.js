// cloudems-toekomst-tabs-card — tab-wrapper v5.5.465
const CLOUDEMS_TOEKOMST_TABS_CARD_VERSION = "5.5.465";

class CloudEMSToekomstTabsCard extends HTMLElement {
  constructor() {
    super();
    ...
    this._setting = false;
    this._tabs = [
    {id:'shadow', label:'🌑 Future Shadow', type:'custom:cloudems-future-shadow-card'},
    {id:'blackout', label:'⚡ Blackout', type:'custom:cloudems-blackout-card'}
    ];
  }
  ...
  static getConfigElement(){return document.createElement('cloudems-toekomst-tabs-card-editor');}
  set hass(h) {
    this._hass = h;
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-shadow' data-tab='shadow'>🌑 Future Shadow</div>
            <div class='tab' id='tab-blackout' data-tab='blackout'>⚡ Blackout</div>
        </div>
            <div id='slot-shadow'></div>
            <div id='slot-blackout'></div>
      </div>`;
...
class CloudemsToekomstTabsCardEditor extends HTMLElement{
...
if (!customElements.get('cloudems-toekomst-tabs-card'))
  customElements.define('cloudems-toekomst-tabs-card', CloudEMSToekomstTabsCard);
window.customCards = window.customCards || [];
if (!window.customCards.find(c => c.type === 'cloudems-toekomst-tabs-card'))
  window.customCards.push({ type: 'cloudems-toekomst-tabs-card', name: 'CloudEMSToekomstTabsCard' });
console.info(`%c CLOUDEMS-TOEKOMST-TABS-CARD %c v${CLOUDEMS_TOEKOMST_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
