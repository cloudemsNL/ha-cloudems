...
class CloudEMSFaseTabsCard extends HTMLElement {
  constructor() {
    ...
    this._setting = false;
    this._tabs = [
    {id:'balans', label:'⚡ Balans', type:'custom:cloudems-fase-balans-card'},
    {id:'historiek', label:'📈 Historiek', type:'custom:cloudems-fase-historiek-card'}
    ];
  }
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-balans' data-tab='balans'>⚡ Balans</div>
            <div class='tab' id='tab-historiek' data-tab='historiek'>📈 Historiek</div>
        </div>
            <div id='slot-balans'></div>
            <div id='slot-historiek'></div>
      </div>`;
...
if (!window.customCards.find(c => c.type === 'cloudems-fase-tabs-card'))
  window.customCards.push({ type: 'cloudems-fase-tabs-card', name: 'CloudEMSFaseTabsCard' });
console.info(`%c CLOUDEMS-FASE-TABS-CARD %c v${CLOUDEMS_FASE_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
