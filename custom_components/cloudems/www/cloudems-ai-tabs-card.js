// cloudems-ai-tabs-card — tab-wrapper v5.5.465
const CLOUDEMS_AI_TABS_CARD_VERSION = "5.5.465";

class CloudEMSAiTabsCard extends HTMLElement {
  constructor() {
    super();
    ...
    this._setting = false;
    this._tabs = [
    {id:'ai', label:'🧠 AI Status', type:'custom:cloudems-ai-card'},
    {id:'leerresultaat', label:'📊 Leerresultaten', type:'custom:cloudems-decisions-learner-card'},
    {id:'leerproces', label:'📈 Leerproces', type:'custom:cloudems-learning-card'}
    ];
  }
  ...
  static getConfigElement(){return document.createElement('cloudems-ai-tabs-card-editor');}
  set hass(h) {
    this._hass = h;
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-ai' data-tab='ai'>🧠 AI Status</div>
            <div class='tab' id='tab-leerresultaat' data-tab='leerresultaat'>📊 Leerresultaten</div>
            <div class='tab' id='tab-leerproces' data-tab='leerproces'>📈 Leerproces</div>
        </div>
            <div id='slot-ai'></div>
            <div id='slot-leerresultaat'></div>
            <div id='slot-leerproces'></div>
      </div>`;
...
class CloudemsAiTabsCardEditor extends HTMLElement{
...
if (!customElements.get('cloudems-ai-tabs-card'))
  customElements.define('cloudems-ai-tabs-card', CloudEMSAiTabsCard);
window.customCards = window.customCards || [];
if (!window.customCards.find(c => c.type === 'cloudems-ai-tabs-card'))
  window.customCards.push({ type: 'cloudems-ai-tabs-card', name: 'CloudEMSAiTabsCard' });
console.info(`%c CLOUDEMS-AI-TABS-CARD %c v${CLOUDEMS_AI_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
