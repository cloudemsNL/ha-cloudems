// cloudems-nilm-visual-tabs-card — tab-wrapper v5.5.465
const CLOUDEMS_NILM_VISUAL_TABS_CARD_VERSION = "5.5.465";

class CloudEMSNilmVisualTabsCard extends HTMLElement {
  constructor() {
    super();
    ...
    this._setting = false;
    this._tabs = [
    {id:'energie', label:'⚡ Energie', type:'custom:cloudems-energy-visual-card'},
    {id:'nilm', label:'🔍 NILM', type:'custom:cloudems-nilm-visual-card'}
    ];
  }
  ...
  static getConfigElement(){return document.createElement('cloudems-nilm-visual-tabs-card-editor');}
  set hass(h) {
    this._hass = h;
  ...
  async _setup() {
    ...
    sh.innerHTML = `
      ...
      <div id="shell">
        <div class="tab-bar">
            <div class='tab' id='tab-energie' data-tab='energie'>⚡ Energie</div>
            <div class='tab' id='tab-nilm' data-tab='nilm'>🔍 NILM</div>
        </div>
            <div id='slot-energie'></div>
            <div id='slot-nilm'></div>
      </div>`;
...
class CloudemsNilmVisualTabsCardEditor extends HTMLElement{
...
if (!customElements.get('cloudems-nilm-visual-tabs-card'))
  customElements.define('cloudems-nilm-visual-tabs-card', CloudEMSNilmVisualTabsCard);
window.customCards = window.customCards || [];
if (!window.customCards.find(c => c.type === 'cloudems-nilm-visual-tabs-card'))
  window.customCards.push({ type: 'cloudems-nilm-visual-tabs-card', name: 'CloudEMSNilmVisualTabsCard' });
console.info(`%c CLOUDEMS-NILM-VISUAL-TABS-CARD %c v${CLOUDEMS_NILM_VISUAL_TABS_CARD_VERSION} `, 'background:#60a5fa;color:#000;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'background:#0d1117;color:#60a5fa;font-weight:700;padding:2px 6px;border-radius:0 3px 3px 0');
